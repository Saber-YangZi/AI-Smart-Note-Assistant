...
async def word_loader(file_path: str) -> list[Document]:
    ...
    try:
        ...
        logger.warning("【WORD 文件加载】Docx2txtLoader 不可用，尝试使用 python-docx")
        try:
            import docx
            ...
            logger.error(f"【WORD 文件加载】使用 python-docx 加载失败：{e}")
            return []
    except Exception as e:
        logger.error(f"【WORD 文件加载】加载文件 {abs_file_path} 时出错：{e}")
        return []
